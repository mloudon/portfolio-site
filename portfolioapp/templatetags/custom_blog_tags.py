...
def blog_months(*args):
    ...
    dates = blog_posts.values_list("publish_date", flat=True)
...
def blog_categories(*args):
    ...
    categories = categories.exclude(title__iexact="projects")
...
def blog_recent_posts(limit=5, tag=None, username=None, category=None):
    ...
    if (not category) or (category.upper() != "projects".upper()):
        project_category = BlogCategory.objects.filter(title__iexact="projects")
        if project_category.exists():
            blog_posts = blog_posts.exclude(categories__in=project_category)
